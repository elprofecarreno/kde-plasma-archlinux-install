...
detect_bootloader()
{
    ...
    echo -e "${GREEN}[ OK ]${RESET} Tipo: $BOOT_TYPE"
}
...
configure_nvidia_initramfs() {
    echo -e "\n${CYAN}==> Configurando NVIDIA en initramfs...${RESET}"
    sed -i \
...
EOF
    echo -e "${GREEN}[ OK ] NVIDIA agregado a initramfs${RESET}"
}


configure_systemd_boot_uki() {
    echo -e "\n${CYAN}==> Configurando UKI systemd-boot...${RESET}"
    CMDLINE="/etc/kernel/cmdline"

    if [ -f "$CMDLINE" ]; then
        if ! grep -q "nvidia_drm.modeset=1" "$CMDLINE"; then
            sed -i 's/$/ nvidia_drm.modeset=1/' "$CMDLINE"
            echo -e "${GREEN}[ OK ] Parámetro NVIDIA agregado${RESET}"
        fi
    else

cat > "$CMDLINE" <<EOF
root=PARTUUID=a017776b-76a8-4cd3-ba09-afd4da77db7a rw rootfstype=ext4 nvidia_drm.modeset=1
EOF
